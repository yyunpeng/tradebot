...
for t in range(30, 239, 2):
    ...
    dim, mean_matrix_list, covariance_matrix_list = simulator.execute(t=t, tau_limit=10, r=0.1, path_num=50) # path_num = 200 will take 17.87 seconds, not too bad
    ...
    for stock_index in range(100):
        # algo_checker_sell = Algo_Check(E_S=mean_matrix_list[stock_index], 
        #                           Sigma=covariance_matrix_list[stock_index], 
        #                           dim=dim,
        #                           upper_bound=0, 
        #                           lower_bound=-50, 
        #                           number_of_share=50, 
        #                           number_of_iterations=15) # Number of iterations for minimization. Default is 1000 but will take 4-7 seconds which is too long. Restricting to 25 iterations, each iteration only takes 0.14-0.16 seconds which is much better.
        # x_estimate_sell = algo_checker_sell.execute()
        # algo_checker_buy = Algo_Check(E_S=mean_matrix_list[stock_index], 
        #                           Sigma=covariance_matrix_list[stock_index], 
        #                           dim=dim, 
        #                           upper_bound=50, 
        #                           lower_bound=0, 
        #                           number_of_share=50, 
        #                           number_of_iterations=15) # Number of iterations for minimization. Default is 1000 but will take 4-7 seconds which is too long. Restricting to 25 iterations, each iteration only takes 0.14-0.16 seconds which is much better.
        # x_estimate_buy = algo_checker_buy.execute()
        # print('t = {}; Stock: {}; x_estimate_sell: {}'.format(t, stock_index, x_estimate_sell))
        # print('t = {}; Stock: {}; x_estimate_buy: {}'.format(t, stock_index, x_estimate_buy))
        # print("x_estimate_sell = {}".format(x_estimate_sell))
        # print("x_estimate_buy = {}".format(x_estimate_buy))
        # if x_estimate_sell[0] < 0.:
        #     sell_arr[stock_index,t] = abs(x_estimate_sell[0])
        # elif x_estimate_buy[0] > 0.:
        #     buy_arr[stock_index,t] = abs(x_estimate_buy[0])
        # del algo_checker_sell, algo_checker_buy
        algo_checker = Algo_Check(E_S=mean_matrix_list[stock_index], 
                                  ...
                                  lower_bound=-50, 
                                  number_of_share=50, 
                                  number_of_iterations=25) # Number of iterations for minimization. Default is 1000 but will take 4-7 seconds which is too long. Restricting to 25 iterations, each iteration only takes 0.14-0.16 seconds which is much better.
...
for i in range(100):
    ...
    if row_sum == 0:
        index_arr = np.random.choice(105,97, replace=False)
        index_arr = np.unique(index_arr)
        for i in range(index_arr.shape[0]):
            indexing_inside = index_arr[i]
            indexing_inside = indexing_inside * 2 + 30
            extracted_array_original[indexing_inside] += 1
        row_sum = np.sum(extracted_array_original)
