...
def dfs(lst, selected_tuples, total_sum=0, last_val=-60):
    if not lst:
        return total_sum
    
    for i, sublst in enumerate(lst):
        if sublst:
            sublst_sorted = sorted(sublst, key=lambda x: x[2], reverse=True)
            for tup in sublst_sorted:
                if tup[1] - last_val >= 61:
                    total_sum += tup[2]
                    selected_tuples.append(tup)
                    last_val = tup[1]
                    dfs(lst[i+1:], selected_tuples, total_sum, last_val)
                    return total_sum
            break
