...
logging.info("This entire code file is going to take approximately [40 mins - 1 hour] to run.\nWe thank you for your patience.")
...
logging.info("The importing process took {:.2f} seconds.".format(end-start))
...
logging.info("Creating tick_data pandas framework took {:.2f} seconds.".format(end-start))

# ---------- Creating assetPrices numpy array, dimension: 100 X 14402; assetPrices_perMinute numpy array, dimension: 100 X 240  ----------

logging.info("Creating asset prices per minute interval NumPy array.\nPlease wait, it will take approximately 3 minutes...")
...
logging.info("Creating assetPrices and assetPrices_perMinute numpy array took {:.2f} seconds.".format(end-start))
...
for t in range(30, 239, 2):
    ...
    logging.info("t = {}. Start simulating stock movement paths.".format(t))
    ...
    logging.info("Simulation at Minute_Index = '{}' took {:.2f} seconds.".format(t, end_simulate-start_simulate))
    ...
    for stock_index in range(100):
        ...
        algo_checker = Algo_Check(E_S=mean_matrix_list[stock_index], 
                                  ...
                                  dim=dim, 
                                  upper_bound=10, #upperbound是正数
                                  lower_bound=-10, #lowerbound是负数 不需要改正负号 直接换数字
                                  number_of_share=50, 
                                  number_of_iterations=25) 
        x_estimate = algo_checker.execute()
        logging.info("t={}. stock_index = {}. x_estimate = {}".format(t, stock_index, x_estimate))
        
        df = pd.DataFrame({'E_S': mean_matrix_list[stock_index]})
        rsi = ta.momentum.RSIIndicator(df['E_S'], window=10).rsi()
    ...
    logging.info("Simulation at Minute_Index = '{}' took {:.2f} seconds.".format(t, end_simulate-start_simulate))
    logging.info("Running minimisation algorithm at Minute_Index = '{}' took {:.2f} seconds.".format(t, end_algo-start_algo)) # Algorithm takes 33 seconds, not too bad
    del mean_matrix_list, covariance_matrix_list

logging.info(sell_arr)
logging.info(buy_arr)

logging.info("Working in progress...")
...
for stock_index in range(0,100):
    for minute_index in range(240): 
        if buy_tick_list[stock_index][minute_index]==[]:
            ...
            max_second = 999999999
            max_tuple = ()
            for tup in buy_tick_list[stock_index][minute_index]:
                if tup[2] < max_second and tick_data.iloc[tup[0],8]!=1:
                    # print(tup)
                    max_second = tup[2]
                    max_tuple = tup
...
logging.info("The algorithm process took {:.2f} seconds.".format(end-start))

# ---------- Writer ----------

start = time.perf_counter()

logging.info("Start writing to files...")
...
logging.info("The writing process took {:.2f} seconds.".format(end-start))

# ---------- Post Processing ----------

end_main = time.perf_counter()
logging.info("Overall, the entire process took {:.2f} seconds.".format(end_main-start_main))
